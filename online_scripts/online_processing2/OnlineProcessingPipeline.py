...
class OnlineProcessingPipeline:
    ...
    def run_pipeline_dummy(chunk, filters, mov_avg_filter, dec_filter):
        ...
        # CAR - Rereferencing
        filtered_chunk = filtered_chunk - np.nanmean(filtered_chunk, axis=0, keepdims=True)
    ...
    def run_pipeline_sLDA_bp_ObM(chunk, filters, mov_avg_filter, dec_filter):
        # Transpose the chunks into dimensions of (channels x samples)
        chunk = np.transpose(chunk)
        ################################################################################################################
        ###                             Begin processing pipeline                                                    ###
        ...
        # #downsampling
        downsampled_eeg_chunk = dec_filter.decimate_by_dwnf(mov_avg_chunk)
        ################################################################################################################
        ###                                  End Processing pipeline                                                 ###
        ################################################################################################################
